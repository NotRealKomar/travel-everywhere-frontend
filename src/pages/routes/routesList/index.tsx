...
export const RoutesList: React.FC<RoutesListProps> = ({
  ...
  const { routesData, searchQuery, loadRoutesData, onSearchValueChange } =
  ...
  return routesData.length > 0 ? (
    <ListWrapper>
      ...
      {routesData.map((value) => (
        <Travel key={value.id} small={small} {...value} />
      ))}
    </ListWrapper>
  ) : (
    <Box>
      ...
      <EmptyListWrapper>
        ...
        <Typography variant="h4">Нет созданных маршрутов</Typography>
        <Typography
          variant="subtitle2"
          style={{ color: 'gray', margin: '24px' }}
        >
          Создайте новый маршрут через меню &quot;Путешествия&quot; или по{' '}
          <u>ссылке</u>
        </Typography>
      </EmptyListWrapper>
    </Box>
